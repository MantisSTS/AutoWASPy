...
class ASVSService:
    ...
    def get_cached_asvs_data():
        """Get ASVS data from JSON cache file, fallback to hardcoded data if not available"""
        import os
        import json
        
        print("Getting cached ASVS data for project creation...")
        
        # Try to load from cache file first
        cache_file = os.path.join(os.path.dirname(__file__), '..', '..', 'cache', 'asvs_cache.json')
        try:
            if os.path.exists(cache_file):
                with open(cache_file, 'r', encoding='utf-8') as f:
                    cached_data = json.load(f)
                    if cached_data and len(cached_data) > 10:
                        print(f"Using cached ASVS data: {len(cached_data)} tests")
                        return cached_data
        except Exception as e:
            print(f"Error loading ASVS cache file: {e}")
        
        # Fall back to hardcoded data
        print("Using fallback ASVS data")
        return ASVSService._get_fallback_data()
    ...
    def _fetch_from_github():
        ...
        if unique_requirements:
            ...
            # Save to cache file
            ASVSService._save_to_cache(unique_requirements)
    ...
    def _save_to_cache(data):
        """Save fetched ASVS data to JSON cache file"""
        import os
        import json
        ...
        try:
            ...
            # Save to cache file
            cache_file = os.path.join(cache_dir, 'asvs_cache.json')
            with open(cache_file, 'w', encoding='utf-8') as f:
                json.dump(data, f, indent=2, ensure_ascii=False)
            
            print(f"Saved {len(data)} ASVS requirements to cache file: {cache_file}")
        except Exception as e:
            print(f"Error saving ASVS data to cache: {e}")
